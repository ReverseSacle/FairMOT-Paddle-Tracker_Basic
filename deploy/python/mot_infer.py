...
from deploy.python.preprocess import preprocess, NormalizeImage, Permute
from deploy.python.mot_preprocess import LetterBoxResize

from deploy.python.tracker import JDETracker
...
from deploy.python.infer import get_test_images, print_arguments
...
def predict_video(detector,
    ...
    parser = argsparser(new_threshold,
                        new_video_path,
                        new_camera_id)
    FLAGS = parser.parse_args()
    if new_camera_id != -1:
        capture = cv2.VideoCapture(new_camera_id)
